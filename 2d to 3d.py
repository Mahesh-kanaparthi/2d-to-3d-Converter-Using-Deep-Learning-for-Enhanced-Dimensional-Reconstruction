...
class MultiViewDataset(Dataset):
 def _init_(self, root_dir, transform=None, img_size=224, num_points=2048, 
voxel_size=64):
 self.root_dir = root_dir
 self.num_points = num_points
 self.voxel_size = voxel_size
 self.transform = transform or transforms.Compose(transforms.Resize((img_size, 
img_size)),transforms.RandomRotation(30),transforms.RandomAffine(degrees=0,translate=(
0.15,0.15),scale=(0.8,1.2)),transforms.ColorJitter(brightness=0.4,contrast=0.4,saturation=0.4,
hue=0.2),transforms.RandomCrop(img_size,padding=10),transforms.ToTensor(),transforms
Normalize(mean=[0.485,0.456,0.406],std=[0.229,0.224,0.225]),transforms.RandomErasing(p
=0.3, scale=(0.02, 0.1)), ])
 ...
 for i in range(1000): # Increased to 1000 samples
 ...
 color_map = {'cube': (200, 50, 50), 'sphere': (50, 200, 50), 'cylinder': (50, 50, 200), 'cone': 
(200, 200, 50), 'torus': (200, 50, 200), 'pyramid': (100, 100, 50), 'prism': (50, 100, 100), 
'ellipsoid': (100, 50, 100) }
 if shape_type == 'cube':
 pts = np.array([center[0]-size, center[1]-size], [center[0]+size, center[1]-size], 
[center[0]+size, center[1]+size], [center[0]-size, center[1]+size] dtype=np.int32)
 ...
 pts = np.array([[center[0], center[1]-size], [center[0]+size, center[1]+size], [center[0]-
size, center[1]+size]], dtype=np.int32)
 cv2.fillPoly(mask, [pts], 255)
...
size, center[1]+size] dtype=np.int32)
 ...
 pts=np.array([[center[0]-size,center[1]-size],[center[0]+size,center[1]-
size],[center[0]+int(size*1.5),center[1]],[center[0]+size,center[1]+size],[center[0]-
size,center[1]+size], [center[0]-int(size*1.5), center[1]] dtype=np.int32)
 cv2.fillPoly(mask, [pts], 255)
...
class EnhancedPix2Vox(nn.Module):
 def _init_(self, voxel_size=64):
 super(EnhancedPix2Vox, self)._init_()
 self.voxel_size = voxel_size
 ...
 self.view_attention = nn.Sequential(nn.Linear(2048, 512),nn.ReLU(),nn.Dropout(0.5), 
nn.Linear(512, 5),.Softmax(dim=1))
 self.decoder = nn.Sequential(nn.Linear(2048, 2048), nn.ReLU(),nn.BatchNorm1d(2048), 
nn.Dropout(0.5),nn.Linear(2048,8*8*8*32),nn.ReLU(),.BatchNorm1d(8*8*8*32),nn.Unflatte
n(1,(32,8,8,8)),nn.Conv3d(32,16,kernel_size=3,padding=1), nn.ReLU(),nn.BatchNorm3d(16), 
nn.Upsample(scale_factor=2, mode='nearest'), nn.Conv3d(16, 8, kernel_size=3, padding=1), 
nn.ReLU(),nn.BatchNorm3d(8), nn.Upsample(scale_factor=2, mode='nearest'), nn.Conv3d(8, 
4, kernel_size=3, padding=1), nn.ReLU(),nn.BatchNorm3d(4), nn.Upsample(scale_factor=2, 
mode='nearest'), nn.Conv3d(4, 1, kernel_size=3, padding=1))
 def forward(self, x):
 batch_size = x.size(0)
 view_features = []
 for v in range(5):
...
def train_pix2vox(model, train_loader, val_loader, epochs=100, lr=0.0005, accum_steps=2):
 optimizer = Adam(model.parameters(), lr=lr, weight_decay=1e-4) # Increased weight decay
...
train_losses = []
 ...
 dice_loss = 1 - (2 * (outputs_sigmoid * target).sum() / ((outputs_sigmoid + 
target).sum() + 1e-8))
 focal = focal_loss(outputs, target)
 iou = iou_loss(outputs, target)
 loss = 0.3 * bce_loss + 0.3 * dice_loss + 0.2 * focal + 0.2 * iou # Adjusted weights
...
def post_process_voxels(voxels, sigma=0.5):
 ...
 threshold = np.mean(smoothed) + np.std(smoothed) # Adaptive threshold
...
verts = (verts / voxels.shape[0]) - 0.5
 fig = go.Figure(data=[go.Mesh3d(x=verts[:, 0], y=verts[:, 1], z=verts[:, 2], i=faces[:, 0], 
j=faces[:, 1], k=faces[:, 2], color='lightblue', opacity=0.8, flatshading=True)])
fig.update_layout(scene=dict(xaxis=dict(visible=False),yaxis=dict(visible=False),zaxis=dict(v
isible=False), aspectmode='data'), width=800, height=600, title='3D Voxel Reconstruction')
 if save_path:
...
class ReconstructionPipeline:
 ...
 self.pix2vox.eval()
self.transform=transforms.Compose([transforms.Resize((224,224)),transforms.ToTensor(),tra
nsforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),])
...
def analyze_model_performance(model, loader):
 ...
 target = targets['voxels'].to(device)
 with torch.amp.autocast('cuda'):
 outputs = model(images).squeeze(1)outputs_sigmoid = torch.sigmoid(outputs)
 pred_bin = (outputs_sigmoid > 0.5).float()
 ...
 sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Empty', 'Occupied'], 
yticklabels=['Empty', 'Occupied'])
 ...
 print(classification_report(target_bin, pred_bin, target_names=['Empty', 'Occupied']))
 accuracy = (all_preds == all_targets).float().mean().item()
 print(f"\nOverall Voxel Accuracy: {accuracy:.4f}")
 return accuracy
def custom_collate(batch):
 images = torch.stack([item[0] for item in batch])
 targets = {'pointcloud': torch.stack([item[1]['pointcloud'] for item in batch]), 'voxels': 
torch.stack([item[1]['voxels'] for item in batch]) }
 return images, targets
def main():
 voxel_size = 64
 
dataset=MultiViewDataset('/content/shapenet_sample',num_points=2048,voxel_size=voxel_si
ze)
...
collate_fn=custom_collate, num_workers=2)
 ...
 train_losses, val_losses, metrics_history = train_pix2vox(pix2vox, train_loader, val_loader, 
epochs=100, lr=0.0005, accum_steps=2)
 print("\nTraining Curves:")
...
plt.plot(val_losses, label='Val Loss')
 ...
 if accuracy < 0.9:
 print(f"Warning: Voxel accuracy ({accuracy:.4f}) is below 90%. Consider training for 
more epochs or tuning hyperparameters.")
